...
function BSComp({ comp, prev }) {
  ...
  const items = [
    { label: 'Total Assets', k: 'total_assets_change' },
    { label: 'Liabilities', k: 'total_liabilities_change' },
    { label: 'Equity', k: 'equity_change' },
    { label: 'Working Capital', k: 'working_capital_change' },
  ];
  return (
...
export default function Dashboard({ results, onReset, sourceFiles, industry }) {
  ...
  const visibleTabs = ALL_TABS.filter(t => t.id !== 'tax' || !!tax);
  ...
  return (
    <div ref={dashboardRef} style={{ minHeight: '100vh', background: '#F7F4EE' }}>
      ...
      {/* Tab nav */}
      <div style={{ background: '#FFFFFF', borderBottom: '1px solid #E2DDD4', overflowX: 'auto', display: 'flex', scrollbarWidth: 'none' }}>
        {visibleTabs.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            style={{ padding: '12px 15px', background: 'transparent', border: 'none', borderBottom: tab === t.id ? '3px solid #C41E3A' : '3px solid transparent', color: tab === t.id ? '#C41E3A' : '#8A7F70', fontSize: 12, fontFamily: 'IBM Plex Sans', fontWeight: tab === t.id ? 600 : 400, cursor: 'pointer', letterSpacing: '0.02em', marginBottom: -1, whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: 4 }}>
        ...
        {tab === 'insights' && (
          ins.length > 0 ? <InsightsPanel insights={ins} /> : (
            <div style={{ background: '#FFFFFF', border: '1px solid #E2DDD4', padding: '60px 40px', textAlign: 'center', borderRadius: 4 }}>
              ...
              <p style={{ color: '#8A7F70', fontSize: 13, marginTop: 6, fontFamily: 'IBM Plex Sans' }}>Run a Full Analysis (P&L + Balance Sheet) for comprehensive financial insights.</p>
            </div>
          )
        )}
        {tab === 'benchmark' && <BenchmarkPanel results={results} />}
        {tab === 'scenario'  && <ScenarioPanel  results={results} />}
        {tab === 'risk'      && <RiskPanel       results={results} />}
        {tab === 'runway'    && <RunwayPanel     results={results} />}
        {tab === 'report'    && <NarrativePanel  results={results} />}
        {tab === 'chat'      && <AIChatPanel     results={results} />}
