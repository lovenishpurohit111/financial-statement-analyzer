...
export default function SummaryCards({ results }) {
  ...
  const bs   = results?.analysis || results?.bs_current;
  ...
  if (pl?.summary) {
    ...
    cards.push({ label:'Net Profit',        value: fmt(s.net_profit),       fmtFull: fmtF(s.net_profit),       color: s.net_profit >= 0 ? '52,211,153' : '251,113,133', icon: s.net_profit >= 0 ? '📈' : '📉', sub: s.net_profit >= 0 ? '▲ Profit' : '▼ Loss', delay:'0.2s' });
    cards.push({ label:'Gross Margin',      value: pct(r.gross_margin),     color:'34,211,238',  icon:'📊', sub:'Gross', delay:'0.3s' });
    cards.push({ label:'Net Profit Margin', value: pct(r.net_profit_margin),color: r.net_profit_margin >= 10 ? '52,211,153' : r.net_profit_margin >= 0 ? '251,191,36' : '251,113,133', icon:'🎯', delay:'0.35s' });
    cards.push({ label:'Total Expenses',    value: fmt(s.total_cogs + s.total_op_expenses), fmtFull: fmtF(s.total_cogs + s.total_op_expenses), color:'251,113,133', icon:'💸', sub:'Costs', delay:'0.4s' });
    cards.push({ label:'Expense Ratio',     value: pct(r.expense_ratio),    color: r.expense_ratio < 75 ? '52,211,153' : r.expense_ratio < 90 ? '251,191,36' : '251,113,133', icon:'📉', delay:'0.45s' });
  }
  ...
  if (bs?.summary) {
    ...
    cards.push({ label:'Equity',            value: fmt(s.equity),           fmtFull: fmtF(s.equity),           color: s.equity >= 0 ? '52,211,153' : '251,113,133', icon:'💎', delay:'0.2s' });
    cards.push({ label:'Working Capital',   value: fmt(s.working_capital),  fmtFull: fmtF(s.working_capital),  color: s.working_capital >= 0 ? '52,211,153' : '251,113,133', icon:'🔄', delay:'0.25s' });
