        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            <button className="text-xs text-slate-500 hover:text-slate-300 underline"
              onClick={(e) => { e.stopPropagation(); onParsed(slotKey, null); }}>
              Remove
            </button>
          </div>
        ) : (
          <div className="flex flex-col items-center gap-2">
            ...
            <p className="text-slate-600 text-xs">CSV or XLSX</p>
          </div>
        )}
      </div>
      {error && <p className="mt-2 text-xs text-rose-400 px-1">{error}</p>}
    </div>
  );
}
...
export default function UploadPage({ onAnalysisDone }) {
  ...
  const inputStyle = { background:'#0f172a', border:'1px solid #334155', borderRadius:8, color:'#e2e8f0', padding:'8px 12px', fontSize:13, width:'100%', outline:'none' };
      ...
      <div className="glass w-full max-w-2xl p-6 fade-up-2">
        ...
        <div className="grid grid-cols-2 gap-3 mb-5 pt-4" style={{ borderTop:'1px solid rgba(51,65,85,0.4)' }}>
          <div>
            ...
            {!entity && (
              <p className="text-xs text-amber-500/70 mt-1">Add entity type to unlock tax insights</p>
            )}
          </div>
          <div>
        ...
        <button onClick={analyze} disabled={!canAnalyze || analyzing}
          ...
          {analyzing ? 'Analyzing...' : `Run ${mode === 'full' ? 'Full' : 'Quick'} Analysis →`}
        </button>
      </div>

      {/* Hint */}
      <p className="mt-6 text-slate-600 text-xs text-center fade-up-3">
        Sample files available in the GitHub repo · Supports QuickBooks Online exports (XLSX/CSV)
      </p>
    </div>
  );
}
